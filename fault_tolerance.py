        ...
        **kwargs
        ...
        while retries < self.max_retries:
            try:
                ...
                logger.warning(
                    f"Task {task_id} failed (attempt {retries}/{self.max_retries}): {e}"
                )
    ...
    async def migrate_task(
        ...
        try:
            ...
            if task_data:
                ...
                # 分配到新节点
                success = await scheduler.assign_task_to_node(
                    to_node, task_id, task
                )
    ...
    def get_statistics(self) -> Dict:
        ...
        for status in TaskStatus:
            task_stats[status.value] = sum(
                1 for s in self.task_states.values() if s == status
            )

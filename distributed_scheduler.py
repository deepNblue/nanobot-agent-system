...
class DistributedScheduler:
    ...
    def __init__(self, nodes: List[str], load_balancer: LoadBalancer = None, fault_tolerance: FaultTolerance = None):
        """
        初始化分布式调度器
    ...
    async def schedule_task(self, task: Dict, priority: TaskPriority = TaskPriority.NORMAL) -> str:
        """
        调度任务到最佳节点
        ...
        # 2. 选择最佳节点
        best_node = self.load_balancer.select_node(task=task, nodes=self.node_status)
        ...
        if not self.fault_tolerance.check_circuit(best_node):
            ...
            # 尝试其他节点
            healthy_nodes = {k: v for k, v in self.node_status.items() if self.fault_tolerance.check_circuit(k)}
    ...
    async def fetch_node_status(self, node: str) -> Dict:
        ...
        try:
            ...
            return {"status": "unhealthy", "error": "Timeout", "node_url": node, "last_check": datetime.now().isoformat()}

        except Exception as e:
            return {"status": "unhealthy", "error": str(e), "node_url": node, "last_check": datetime.now().isoformat()}

    async def assign_task_to_node(self, node: str, task_id: str, task: Dict) -> bool:
        """
        分配任务到节点
        ...
        try:
            ...
            async with self.session.post(f"{node}/api/task", json=payload) as response:
                if response.status == 200:
                ...
                async with self.session.get(f"{node}/api/task/{task_id}") as response:
                    if response.status == 200:
                        ...
                        if status == "completed":
                            ...
                            # 保存最终状态
                            await self.fault_tolerance.save_checkpoint(task_id, {"status": "completed", "result": task_status})
                        ...
                        if "checkpoint" in task_status:
                            await self.fault_tolerance.save_checkpoint(task_id, task_status["checkpoint"])
    ...
    def get_cluster_status(self) -> Dict:
        ...
        healthy_nodes = sum(1 for s in self.node_status.values() if s.get("status") == "healthy")

        # 统计任务状态
        active_tasks = sum(1 for t in self.task_assignments.values() if t.get("status") == TaskStatus.RUNNING.value)

        completed_tasks = sum(1 for t in self.task_assignments.values() if t.get("status") == "completed")

        failed_tasks = sum(1 for t in self.task_assignments.values() if t.get("status") == "failed")
    ...
    async def rebalance_tasks(self):
        ...
        # 找出高负载节点
        high_load_nodes = [node for node, load in node_loads.items() if load > avg_load * 1.5]

        # 迁移任务
        for node_url in high_load_nodes:
            # 找到该节点上的任务
            tasks_to_migrate = [
                (task_id, data) for task_id, data in self.task_assignments.items() if data.get("node") == node_url
            ]

            # 迁移部分任务
            for task_id, task_data in tasks_to_migrate[: len(tasks_to_migrate) // 2]:
                # 找到低负载节点
                low_load_nodes = [node for node, load in node_loads.items() if load < avg_load * 0.8 and node != node_url]

                if low_load_nodes:
                    target_node = min(low_load_nodes, key=lambda n: node_loads[n])

                    # 迁移任务
                    success = await self.fault_tolerance.migrate_task(self, task_id, node_url, target_node)
...
class PriorityTaskScheduler(DistributedScheduler):
    ...
    async def schedule_task_with_dependencies(
        self, task: Dict, dependencies: List[str] = None, priority: TaskPriority = TaskPriority.NORMAL
    ) -> str:
        """
        调度带依赖的任务
        ...
        if dependencies:
            # 检查依赖是否满足
            unmet_dependencies = [dep_id for dep_id in dependencies if dep_id not in self.completed_tasks]
            ...
            if unmet_dependencies:
                ...
                logger.info(f"Task {task_id} waiting for dependencies: {unmet_dependencies}")
        ...
        for waiting_task_id, dependencies in self.task_dependencies.items():
            if task_id in dependencies:
                ...
                if await self.check_dependencies(waiting_task_id):
                    ...
                    if task_data:
                        await self.schedule_task(task_data["task"], TaskPriority(task_data.get("priority", 2)))
...
def create_distributed_scheduler(
    nodes: List[str], strategy: str = "weighted_score", checkpoint_dir: str = "./checkpoints"
    ...
    return DistributedScheduler(nodes=nodes, load_balancer=load_balancer, fault_tolerance=fault_tolerance)
...
if __name__ == "__main__":
    ...
    async def test_scheduler():
        ...
        # 测试任务调度
        task = {"description": "Test task", "data": {"key": "value"}}

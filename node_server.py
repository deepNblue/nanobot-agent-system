...
class TaskExecutor:
    ...
    def execute_task(self, task_id: str, task: Dict):
        ...
        # 提交到事件循环
        asyncio.run_coroutine_threadsafe(self._execute_task_async(task_id, task), self.loop)
...
def receive_task():
    ...
    try:
        ...
        # 保存任务
        tasks[task_id] = {**task, "id": task_id, "status": "pending", "received_at": datetime.now().isoformat()}
...
def cancel_task(task_id):
    ...
    return jsonify({"success": True, "task_id": task_id, "status": "cancelled"})
...
def save_task_checkpoint(task_id):
    ...
    try:
        ...
        # 保存检查点
        tasks[task_id]["checkpoint"] = {"data": checkpoint_data, "saved_at": datetime.now().isoformat()}
...
def list_tasks():
    ...
    # 排序（按接收时间倒序）
    filtered_tasks.sort(key=lambda t: t.get("received_at", ""), reverse=True)
...
def get_metrics():
    ...
    try:
        ...
        return jsonify(
            {
                "cpu": {"percent_per_cpu": cpu_percent, "percent_avg": sum(cpu_percent) / len(cpu_percent)},
                "memory": {
                    "total": memory.total,
...
def create_app(config: Dict = None) -> Flask:
    ...
    Returns:
        Flask应用实例
    """
    global node_config, task_executor  # noqa: F824

    if config:
        node_config.update(config)

    # 初始化任务执行器
    task_executor = TaskExecutor(max_concurrent_tasks=node_config["max_concurrent_tasks"])
    task_executor.start()

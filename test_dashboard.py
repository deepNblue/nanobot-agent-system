...
def test_dashboard_broadcast():
    ...
    try:
        # 测试任务更新广播
        dashboard.broadcast_task_update("test-task-123", {"status": "running", "progress": 50, "description": "测试任务"})
        print("✅ 任务更新广播成功")

        # 测试错误广播
        dashboard.broadcast_error({"message": "测试错误消息", "task_id": "test-task-123"})
        print("✅ 错误广播成功")

...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # 创建用户
        user1 = await manager.create_user({"name": "Alice", "email": "alice@example.com", "role": "developer"})

        user2 = await manager.create_user({"name": "Bob", "email": "bob@example.com", "role": "developer"})

        print(f"Created users: {user1.name}, {user2.name}")

        # 创建项目
        project = await manager.create_project(
            {"name": "AI Agent System", "description": "Building an AI-powered development system", "owner": user1.id}
        )

...
"""
        ...
        context = {
            "api_description": "User API endpoints"
        }
...
class TestSuiteManager:
    ...
    async def test_run_suite(self, test_suite_manager):
        ...
        with patch('asyncio.create_subprocess_exec') as mock_exec:
            mock_process = AsyncMock()
            mock_process.communicate.return_value = (
                b"test_unit.py::test_unit_1 PASSED\n2 passed",
                b""
            )
            mock_process.returncode = 0
            mock_exec.return_value = mock_process
...
"""
        ...
        result = await generator.generate_from_file(
            str(src_dir / "math.py"),
            test_type="unit_test",
            output_dir=str(project_dir / "tests")
        )
...
class TestErrorHandling:
    ...
    async def test_model_failure(self, test_generator):
        """测试模型调用失败"""
        # Mock失败的模型调用
        test_generator.model_adapter.call_model = AsyncMock(return_value={
            "success": False,
            "error": "API Error"
        })

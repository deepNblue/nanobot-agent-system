...
class TestSuiteManager:
    ...
    def load_suites(self) -> Dict:
        ...
        logger.info(
            f"加载完成: {len(self.suites)}个文件, "
            f"{sum(s['count'] for s in self.suites.values())}个测试"
        )
    ...
    def get_statistics(self) -> Dict:
        ...
        category_stats = {
            category: {
                ...
                "test_count": sum(self.suites[f]["count"] for f in files if f in self.suites)
            }
    ...
    def organize_tests(self) -> Dict:
        ...
        for filepath, suite in self.suites.items():
            ...
            if not suite["metadata"]["tags"]:
                organization["recommendations"].append({
                    "file": filepath,
                    "recommendation": "缺少标签，建议添加",
                    "priority": "low"
                })
        
        return organization
    
    def export_suite_config(
        self,
        output_file: str = "test_suites.json"
    ) -> str:
        """导出测试套件配置"""
        config = {
...
class TestDependencyAnalyzer:
    ...
    def get_execution_order(self) -> List[str]:
        ...
        # 排序：基础测试优先
        test_files.sort(key=lambda x: (
            "unit" not in x.lower(),
            "integration" not in x.lower(),
            "e2e" not in x.lower(),
            x
        ))

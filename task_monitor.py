...
class TaskMonitor:
    ...
    def __init__(self, tasks_dir: Optional[str] = None):
        ...
        Args:
            tasks_dir: 任务记录目录
        """
        self.tasks_dir = Path(tasks_dir) if tasks_dir else Path.home() / ".nanobot" / "workspace" / "agent_tasks"
    ...
    def check_task_status(self, task_id: str) -> Dict:
        ...
        if not task_info:
            return {"success": False, "error": f"任务不存在: {task_id}", "status": "not_found"}

        # 综合检查
        result = {"task_id": task_id, "timestamp": datetime.now().isoformat(), "overall_status": "unknown", "checks": {}}
        ...
        if task_info.get("worktree"):
            git_status = self._check_git_commits(task_info["worktree"], task_info.get("branch"), task_info.get("startedAt"))
        ...
        if task_info.get("tmuxSession"):
            ...
            result["checks"]["process"] = {"alive": process_alive}
    ...
    def _check_tmux_session(self, session_name: str) -> Dict:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {"exists": False, "running": False, "status": "ended"}

        except Exception as e:
            return {"exists": False, "running": False, "status": "error", "error": str(e)}

    def _check_git_commits(self, worktree_path: str, branch: Optional[str], since_timestamp: Optional[int]) -> Dict:
        """
        检查git提交状态
        ...
        if not worktree.exists():
            return {"success": False, "error": "Worktree不存在", "has_commits": False}
        ...
        try:
            ...
            if branch:
                push_result = subprocess.run(
                    ["git", "branch", "-vv"], cwd=worktree, capture_output=True, text=True, timeout=10
                )
            ...
            return {"success": True, "has_commits": len(commits) > 0, "commit_count": len(commits), "commits": commits[:5]}

        except Exception as e:
            return {"success": False, "error": str(e), "has_commits": False}
    ...
    def _check_ci_status(self, branch: Optional[str]) -> Dict:
        ...
        """
        if not branch:
            return {"success": False, "error": "未指定分支", "ci_status": "unknown"}

        try:
            # 尝试使用gh CLI检查GitHub Actions
            result = subprocess.run(
                ["gh", "run", "list", "--branch", branch, "--limit", "1", "--json", "status,conclusion,createdAt"],
                capture_output=True,
                text=True,
                timeout=30,
            )
            ...
            return {"success": True, "ci_status": "no_runs", "message": "未找到CI运行记录"}

        except FileNotFoundError:
            # gh CLI未安装
            return {"success": False, "error": "gh CLI未安装", "ci_status": "unknown"}
        except Exception as e:
            return {"success": False, "error": str(e), "ci_status": "unknown"}
    ...
    def retry_task(self, task_id: str) -> Dict:
        ...
        if status["overall_status"] not in ["failed", "stuck"]:
            return {"success": False, "error": f"任务状态不允许重试: {status['overall_status']}"}
        ...
        return {"success": True, "task_id": task_id, "retry_count": task_info["retry_count"], "message": "任务已标记为重试"}
    ...
    def _load_task(self, task_id: str) -> Optional[Dict]:
        ...
        """
        # 尝试多种文件名格式
        possible_files = [self.tasks_dir / f"{task_id}.json", self.tasks_dir / f"task_{task_id}.json"]
...
"""
    ...
    async def auto_monitor_with_ci(self, task_id: str) -> Dict:
        ...
        result = {"task_id": task_id, "checks": [], "actions_taken": []}
    ...
    async def continuous_monitoring(self, interval: int = 300, max_duration: int = 7200) -> Dict:
        """
        持续监控所有任务
            ...
            for task in all_tasks:
                ...
                if status in ["running", "needs_review"]:
                    ...
                    monitoring_log.append(
                        {"timestamp": datetime.now().isoformat(), "task_id": task_id, "result": monitor_result}
                    )

            # 等待下一次检查
            await asyncio.sleep(interval)

        return {"success": True, "duration": elapsed, "checks_performed": len(monitoring_log), "log": monitoring_log}

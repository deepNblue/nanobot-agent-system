...
from best_practices_library import (
    BestPracticesLibrary,
    Practice,
    PracticeUsage,
    initialize_default_practices
)
...
def mock_model_adapter():
    """模拟模型适配器"""
    adapter = Mock()
    adapter.call_model = AsyncMock(return_value={
        "success": True,
        "content": json.dumps({
            "type": "find_similar",
            "target": "test",
...
class TestCodeKnowledgeGraph:
    ...
    @pytest.mark.asyncio
    async def test_query_with_model_adapter(
        self,
        knowledge_graph,
        temp_codebase,
        mock_model_adapter
...
class TestBestPracticesLibrary:
    ...
    async def test_record_usage(self, practices_library):
        ...
        # 记录使用
        usage_id = await practices_library.record_usage(
    ...
    async def test_get_popular_practices(self, practices_library):
        ...
        # 记录多次使用
        for i in range(3):
            await practices_library.record_usage(
                practice_id=practice_id,
    ...
    async def test_import_practices(self, practices_library):
        ...
        practices = [
            {
                "title": "Imported Practice 1",
                "category": "general",
                "description": "Test"
            },
            {
                "title": "Imported Practice 2",
                "category": "general",
                "description": "Test"
            }
        ]
        
        count = await practices_library.import_practices(
...
class TestSmartRecommender:
    ...
    async def test_recommend_for_task(self, setup_data):
        ...
        task = {
            "description": "I need to add error handling to my API"
        }
    ...
    async def test_generate_suggestions(self, setup_data):
        ...
        suggestions = await recommender._generate_suggestions(
            task, requirements, practices
        )
...
class TestKnowledgeBaseIntegration:
    ...
    async def test_full_workflow(self, temp_dir, temp_codebase):
        ...
        result = await recommender.recommend_for_task({
            "description": "Write tests for my code"
        })

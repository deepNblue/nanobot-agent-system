...
class CoverageOptimizer:
    ...
    async def run_coverage(
        self, source_dir: str = ".", test_dir: str = "./tests", extra_args: Optional[List[str]] = None
    ) -> Dict:
        """
        运行覆盖率测试
        ...
        # 1. 构建命令
        cmd = ["pytest", test_dir, f"--cov={source_dir}", "--cov-report=json:coverage.json", "--cov-report=term-missing", "-v"]
    ...
    async def suggest_tests(self, uncovered_lines: Optional[Dict] = None, max_suggestions: int = 10) -> List[Dict]:
        """
        为未覆盖代码生成测试建议
            ...
            try:
                ...
                for line_num in lines:
                    if 0 < line_num <= len(code_lines):
                        uncovered_code.append({"line_number": line_num, "code": code_lines[line_num - 1]})
    ...
    async def optimize_coverage(self, target_coverage: float = 90.0, max_iterations: int = 5, test_generator=None) -> Dict:
        """
        优化测试覆盖率
        ...
        results = {"iterations": [], "initial_coverage": 0, "final_coverage": 0, "improvement": 0, "tests_generated": 0}

        # 初始覆盖率
        initial = await self.run_coverage()
        if not initial.get("success"):
            return {"success": False, "error": "无法运行初始覆盖率测试"}
        ...
        while current_coverage < target_coverage and iteration < max_iterations:
            ...
            if test_generator:
                for suggestion in suggestions:
                    ...
                    try:
                        result = await test_generator.generate_from_file(file_path, suggestion["suggested_test_type"])
            ...
            results["iterations"].append(
                {"iteration": iteration, "coverage": current_coverage, "tests_generated": results["tests_generated"]}
            )
    ...
    def get_coverage_report(self) -> str:
        ...
        # 按覆盖率排序
        files_sorted = sorted(analysis["files_analysis"].items(), key=lambda x: x[1]["coverage_percent"])
        ...
        for file_path, data in files.items():
            ...
            if data.get("summary", {}).get("percent_covered", 0) == 0:
                ...
                if not file_path.startswith("test_") and "/tests/" not in file_path:
                    dead_code.append({"file": file_path, "type": "entire_file", "reason": "文件完全没有被测试覆盖"})
    ...
    async def find_duplicate_tests(self, test_dir: str = "./tests") -> List[Dict]:
                ...
                if file.startswith("test_") and file.endswith(".py"):
                    ...
                    try:
                        ...
                        for func in test_functions:
                            ...
                            test_signatures[func].append({"file": file_path, "function": func})
                    except Exception as e:
        ...
        for func_name, locations in test_signatures.items():
            if len(locations) > 1:
                duplicates.append({"function": func_name, "locations": locations, "count": len(locations)})

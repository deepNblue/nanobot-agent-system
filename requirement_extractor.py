    ...
    async def scan_daily_notes(self, days: int = 7) -> List[Dict]:
        ...
        for note_file in daily_notes_path.glob("*.md"):
            try:
                ...
                notes.append(
                    {"path": str(note_file), "title": note_file.stem, "content": content, "modified_at": mtime.isoformat()}
                )
...
只输出JSON，不要其他说明。"""
        ...
        data = {"model": self.model, "messages": [{"role": "user", "content": prompt}], "max_tokens": 2000, "temperature": 0.3}

        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.api_base, headers=headers, json=data, timeout=aiohttp.ClientTimeout(total=60)
                ) as response:
    ...
    async def extract_requirements(self, days: int = 7) -> List[Dict]:
        ...
        # 2. 筛选包含行动项的笔记
        action_notes = [note for note in notes if self._contains_action_items(note["content"])]

    ...
    async def auto_create_pr(self, task_id: str) -> Dict:
        ...
        try:
            ...
            if task.get("status") != "completed":
                return {
                    "success": False,
                    "error": f"任务未完成，当前状态: {task.get('status')}"
                }
            ...
            if self.require_review:
                ...
                if review_score < self.min_review_score:
                    return {
                        "success": False,
                        "error": f"Code Review未通过（得分: {review_score} < {self.min_review_score}）"
                    }
        ...
        try:
            ...
            if status.get("state") != "OPEN":
                return {
                    "success": False,
                    "pr_number": pr_number,
                    "reason": f"PR状态不是OPEN（当前: {status.get('state')}）"
                }
            ...
            if not result.get("success"):
                return {
                    "success": False,
                    "pr_number": pr_number,
                    "error": f"合并命令执行失败: {result.get('error')}"
                }
...
"""
    ...
    async def add_labels(self, pr_number: int, labels: List[str]):
        ...
        try:
            # 先创建标签（如果不存在）
            for label in labels:
                create_cmd = f"gh label create '{label}' --color '0E8A16' --description 'Agent generated label' 2>/dev/null || true"

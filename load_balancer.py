...
class LoadBalancer:
    ...
    def _record_selection(self, node_url: str, task: Dict):
        ...
        if node_url not in self.node_history:
            self.node_history[node_url] = {"total_selections": 0, "recent_selections": []}

        history = self.node_history[node_url]
        history["total_selections"] += 1
        history["recent_selections"].append({"task_id": task.get("id"), "timestamp": datetime.now().isoformat()})
    ...
    def get_statistics(self) -> Dict:
        """获取负载均衡统计信息"""
        total_selections = sum(h["total_selections"] for h in self.node_history.values())
    ...
    def select_node_with_prediction(self, task: Dict, nodes: Dict) -> Optional[str]:
        ...
        for node_url, status in healthy_nodes.items():
            ...
            if prediction["confidence"] > 0.5:
                ...
                # 加权平均
                final_score = current_score * (1 - prediction["confidence"]) + predicted_score * prediction["confidence"]
            else:
...
if __name__ == "__main__":
    ...
    # 测试普通任务
    task1 = {"id": "task_1", "description": "Normal task"}

    best_node = lb.select_node(task1, nodes)
    print(f"Best node for task1: {best_node}")

    # 测试GPU任务
    task2 = {"id": "task_2", "description": "GPU task", "requires_gpu": True}

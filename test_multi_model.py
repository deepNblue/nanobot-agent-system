...
class TestModelSelector:
    ...
    async def test_select_best_model_chinese(self, selector):
        """测试中文任务选择"""
        task = {
            "description": "编写中文技术文档",
            "type": "documentation"
        }
        
        # 添加语言约束
        constraints = {"include_models": [m for m in selector.available_models 
        ...
        # 如果有支持中文的模型，应该选择一个
        if any("zh" in MODEL_CONFIGS.get(m, {}).get("languages", []) 
               for m in selector.available_models):
            # 检查至少有支持中文的模型可用
            has_chinese_support = any(
                "zh" in MODEL_CONFIGS.get(m, {}).get("languages", [])
                for m in selector.available_models
            )
    ...
    def test_apply_constraints_exclude(self, selector):
        ...
        constraints = {
            "exclude_models": ["gpt-4", "claude-3-opus"]
        }
...
class TestPerformance:
    ...
    async def test_concurrent_calls(self, adapter):
        ...
        if models:
            ...
            tasks = [
                adapter.count_tokens(models[0], f"测试文本 {i}")
                for i in range(10)
            ]

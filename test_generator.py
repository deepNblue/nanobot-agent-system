...
class TestCaseGenerator:
    ...
    def __init__(self, model_adapter):
        ...
        self.generation_stats = {"total_generated": 0, "successful": 0, "failed": 0, "by_type": {}}
...
""",
    ...
    async def generate_tests(
        ...
        try:
            ...
            if not result.get("success"):
                ...
                return {"success": False, "error": result.get("error", "模型调用失败"), "test_type": test_type}
    ...
    def _validate_test(self, test_code: str, test_type: str) -> Dict:
        """验证测试代码的质量"""
        validation = {"is_valid": True, "score": 0, "checks": {}}
    ...
    def _analyze_test_coverage(self, test_code: str, source_code: str) -> Dict:
        """分析测试覆盖范围"""
        analysis = {"test_functions": [], "tested_functions": [], "source_functions": [], "coverage_estimate": 0}
        ...
        if analysis["source_functions"]:
            analysis["coverage_estimate"] = len(analysis["tested_functions"]) / len(analysis["source_functions"]) * 100
    ...
    async def generate_from_file(
        self, file_path: str, test_type: str = "unit_test", output_dir: str = "./tests", context: Optional[Dict] = None
    ) -> Dict:
        """
        从文件生成测试用例
        ...
        saved_path = await self.save_test(result["test_code"], test_filename, output_dir)
    ...
    async def save_test(self, test_code: str, filename: str, output_dir: str = "./tests") -> str:
        """
        保存测试文件
    ...
    async def batch_generate(
        self, files: List[str], test_type: str = "unit_test", output_dir: str = "./tests", max_concurrent: int = 3
    ) -> Dict:
        """
        批量生成测试用例
        ...
        async def generate_with_semaphore(file_path: str):
            async with semaphore:
                return await self.generate_from_file(file_path, test_type, output_dir)
        ...
        # 统计结果
        summary = {"total": len(files), "successful": 0, "failed": 0, "results": []}

        for i, result in enumerate(results):
            if isinstance(result, Exception):
                summary["failed"] += 1
                summary["results"].append({"file": files[i], "success": False, "error": str(result)})
            else:
                if result.get("success"):
                    summary["successful"] += 1
                else:
                    summary["failed"] += 1
                summary["results"].append({"file": files[i], **result})
    ...
    async def improve_test(self, existing_test: str, source_code: str, issues: Optional[List[str]] = None) -> Dict:
        """
        改进现有测试
...
"""
        ...
        result = await self.model_adapter.call_model(model="glm5-turbo", prompt=prompt, temperature=0.3, max_tokens=3000)

        if not result.get("success"):
            return {"success": False, "error": result.get("error", "模型调用失败")}
...
# 便捷函数
async def generate_test_quick(code: str, test_type: str = "unit_test", model_adapter=None) -> Dict:
    """快速生成测试用例"""

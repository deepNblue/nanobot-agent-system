...
class CacheManager:
    ...
    def set(self, key: str, value: Any, persist: bool = False):
        ...
        entry = {"value": value, "timestamp": time.time(), "last_access": time.time()}
    ...
    def _evict_lru(self):
        ...
        # 找到最久未使用的键
        lru_key = min(self.memory_cache.keys(), key=lambda k: self.memory_cache[k]["last_access"])
    ...
    def _load_persistent_cache(self):
        ...
        # 只加载最近的缓存
        cache_files = sorted(Path(self.cache_dir).glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)[
            :100
        ]  # 只加载最近的100个
    ...
    def clear_expired(self):
        ...
        # 清理内存缓存
        expired_keys = [k for k, v in self.memory_cache.items() if current_time - v["timestamp"] > self.ttl]

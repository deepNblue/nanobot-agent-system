...
class BaseModelAdapter(ABC):
    ...
    @abstractmethod
    async def generate(self, prompt: str, temperature: float = 0.7, max_tokens: int = 2000, **kwargs) -> Dict:
        """生成响应"""
        pass
...
class GLM5Adapter(BaseModelAdapter):
    ...
    async def generate(self, prompt: str, temperature: float = 0.7, max_tokens: int = 2000, **kwargs) -> Dict:
        ...
        if "system_prompt" in kwargs:
            data["messages"].insert(0, {"role": "system", "content": kwargs["system_prompt"]})
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    ...
                    return {"success": False, "error": "请求超时，请稍后重试", "model": self.model_id}
                ...
                if attempt == self.max_retries - 1:
                    ...
                    return {"success": False, "error": f"网络错误: {str(e)}", "model": self.model_id}
                ...
                self.error_count += 1
                self.last_error = str(e)
                return {"success": False, "error": f"未知错误: {str(e)}", "model": self.model_id}
    ...
    async def stream_generate(self, prompt: str, temperature: float = 0.7, max_tokens: int = 2000, **kwargs):
        """流式生成（异步生成器）"""
        headers = {"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}
        ...
        try:
            timeout = aiohttp.ClientTimeout(total=self.timeout)
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(f"{self.endpoint}/{self.model_id}/invoke", headers=headers, json=data) as response:
...
class ClaudeAdapter(BaseModelAdapter):
    ...
    async def generate(self, prompt: str, temperature: float = 0.7, max_tokens: int = 2000, **kwargs) -> Dict:
        ...
        data = {"model": self.model_id, "messages": messages, "max_tokens": max_tokens, "temperature": temperature}
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    self.error_count += 1
                    self.last_error = str(e)
                    return {"success": False, "error": f"错误: {str(e)}", "model": self.model_id}
                await asyncio.sleep(2**attempt)
...
class QwenAdapter(BaseModelAdapter):
    ...
    async def generate(self, prompt: str, temperature: float = 0.7, max_tokens: int = 2000, **kwargs) -> Dict:
        ...
        data = {
            "model": self.model_id,
            "input": {"messages": kwargs.get("messages", [{"role": "user", "content": prompt}])},
            "parameters": {"temperature": temperature, "max_tokens": max_tokens},
        }

        if "system_prompt" in kwargs:
            data["input"]["messages"].insert(0, {"role": "system", "content": kwargs["system_prompt"]})
...
class ModelAdapter:
    ...
    async def call_model(self, model: str, prompt: str, temperature: float = 0.7, max_tokens: int = 2000, **kwargs) -> Dict:
        """调用指定模型"""
        if model not in self.adapters:
            available = list(self.adapters.keys())
            logger.error(f"模型 {model} 不可用，可用模型: {available}")
            return {"success": False, "error": f"模型 {model} 不可用或未配置", "available_models": available}

        try:
            adapter = self.adapters[model]
            result = await adapter.generate(prompt=prompt, temperature=temperature, max_tokens=max_tokens, **kwargs)
    ...
    def get_all_stats(self) -> Dict[str, Dict]:
        """获取所有模型统计信息"""
        return {model: adapter.get_stats() for model, adapter in self.adapters.items()}
    ...
    async def batch_call(self, requests: List[Dict], max_concurrent: int = 5) -> List[Dict]:
        """批量调用模型（并发控制）"""
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append({"success": False, "error": str(result), "request_index": i})
            else:
...
# 便捷函数
async def quick_call(prompt: str, model: str = None, **kwargs) -> Dict:
    """快速调用模型"""
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        if adapter.get_available_models():
            ...
            result = await adapter.call_model(model, "你好，请介绍一下你自己", max_tokens=100)

...
class RealtimeCollaboration:
    ...
    async def apply_operation(self, operation: Operation) -> bool:
        ...
        if operation.type == "insert":
            ...
            # 替换内容
            new_content = (
                content[: operation.position] + operation.content + content[operation.position + len(operation.content) :]
            )
    ...
    async def broadcast_operation(self, operation: Operation):
        ...
        if self.websocket_manager:
            ...
            await self.websocket_manager.broadcast(
                document_id=operation.document_id, message=message, exclude_user=operation.user_id
            )
    ...
    async def update_cursor(self, cursor: Cursor):
        ...
        if self.websocket_manager:
            ...
            await self.websocket_manager.broadcast(
                document_id=cursor.document_id, message=message, exclude_user=cursor.user_id
            )
    ...
    async def user_join(self, user_id: str, document_id: str):
        ...
        if self.websocket_manager:
            ...
            await self.websocket_manager.broadcast(document_id=document_id, message=message)
    ...
    async def get_operations(self, document_id: str, since_version: int = None) -> List[Dict]:
        """获取操作历史"""
...
class WebSocketManager:
    ...
    async def broadcast(self, document_id: str, message: Dict, exclude_user: str = None):
        """广播消息"""
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # 应用操作
        operation = Operation(
            id="op_1", user_id="user_1", document_id="doc_1", type="insert", position=5, content=" Beautiful"
        )

...
class ModelSelector:
    ...
    async def select_best_model(self, task: Dict, constraints: Dict = None) -> str:
        """选择最佳模型
    ...
    def needs_creativity(self, task: Dict) -> bool:
        """是否需要创意"""
        creative_keywords = ["设计", "创意", "优化", "改进", "创新", "design", "creative", "optimize", "improve", "innovate"]
        description = task.get("description", "").lower()
    ...
    def apply_constraints(self, features: TaskFeatures, constraints: Dict) -> List[str]:
        """应用约束条件"""
        ...
        if constraints.get("max_cost"):
            ...
            candidates = [m for m in candidates if MODEL_CONFIGS.get(m, {}).get("cost_per_1k_tokens", 999) <= max_cost]

        # 速度约束
        if constraints.get("requires_fast") or features.requires_speed:
            candidates = [m for m in candidates if MODEL_CONFIGS.get(m, {}).get("speed") in ["fast", "very_fast"]]

        # 质量约束
        if constraints.get("requires_high_quality"):
            candidates = [m for m in candidates if MODEL_CONFIGS.get(m, {}).get("quality") in ["high", "very_high"]]

        # 语言约束
        if features.requires_chinese:
            candidates = [m for m in candidates if "zh" in MODEL_CONFIGS.get(m, {}).get("languages", [])]

        # 上下文窗口约束
        if features.estimated_tokens > 0:
            min_context = features.estimated_tokens * 2  # 至少2倍空间
            candidates = [m for m in candidates if MODEL_CONFIGS.get(m, {}).get("context_window", 0) >= min_context]

        # 排除特定模型
        if constraints.get("exclude_models"):
            candidates = [m for m in candidates if m not in constraints["exclude_models"]]

        # 只使用特定模型
        if constraints.get("include_models"):
            candidates = [m for m in candidates if m in constraints["include_models"]]

        return candidates

    def calculate_score(self, model: str, features: TaskFeatures, constraints: Dict) -> float:
        """计算模型得分"""
    ...
    def _score_speed(self, requires_speed: bool, speed: str) -> float:
        ...
        speed_scores = {"very_fast": 1.0, "fast": 0.9, "medium": 0.5, "slow": 0.2}
    ...
    async def get_model_recommendations(self, task: Dict, top_n: int = 3) -> List[Tuple[str, float, str]]:
        """获取模型推荐列表
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        # 测试2：快速任务
        task2 = {"description": "快速修复登录页面的样式问题", "priority": "high"}
        ...
        # 测试3：中文任务
        task3 = {"description": "编写一份中文技术文档", "type": "documentation"}

...
class AgentCLI:
    ...
    async def status(self, args):
        ...
        if args.task_id:
            ...
            for task in tasks:
                status_icon = {"running": "🔄", "completed": "✅", "failed": "❌", "needs_review": "⚠️"}.get(
    ...
    async def capture(self, args):
        """捕获任务输出"""
        result = await self.orchestrator.capture_task_output(task_id=args.task_id, lines=args.lines)
    ...
    async def intervene(self, args):
        ...
        result = await self.orchestrator.intervene_task(task_id=args.task_id, command=args.command, enter=not args.no_enter)
    ...
    async def complete(self, args):
        ...
        result = await self.orchestrator.complete_task(task_id=args.task_id, cleanup=args.cleanup)
...
示例:
  ...
  agent-cli attach codex-feat-xxx
    ...
    parser.add_argument("--repo", help="Git仓库路径（默认：当前目录）")

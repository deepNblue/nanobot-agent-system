...
def run_tests():
    ...
    # 测试发送命令
    if exists:
        send_result = tmux_manager.send_command(
            session_name=test_session,
        ...
        # 测试捕获输出
        capture_result = tmux_manager.capture_pane(
            session_name=test_session,
            lines=50
        )
        ...
        results.add_result(
            "列出所有会话",
            test_session in session_names,
            f"找到的会话: {', '.join(session_names)}"
        )
        ...
        results.add_result(
            "终止会话",
            kill_result.get("success", False),
            kill_result.get("error", "")
        )
    ...
    create_result = worktree_manager.create_worktree(
    ...
    if create_result.get("success"):
        ...
        results.add_result(
            "列出所有Worktree",
            str(worktree_path) in worktree_paths,
            f"找到 {len(worktrees)} 个worktree"
        )
    ...
    results.add_result(
        "生成监控报告",
        len(report) > 0 and "任务监控报告" in report,
    ...
    # 1. 创建worktree
    worktree_result = worktree_manager.create_worktree(
    ...
    if worktree_result.get("success"):
        ...
        tmux_result = tmux_manager.create_session(
            session_name=f"test-{integration_task_id}",
            working_dir=worktree_path,
            command="echo 'Integration test started'"
        )
        ...
        if tmux_result.get("success"):
            ...
            results.add_result(
                "集成-监控任务",
                "overall_status" in monitor_result,
                f"状态: {monitor_result.get('overall_status')}"
            )
            
            # 5. 发送干预命令
            intervene_result = tmux_manager.send_command(
                session_name=session_name,
                command="echo 'Intervention test'"
            )
            results.add_result(
                "集成-干预命令",
                intervene_result.get("success", False),
                intervene_result.get("error", "")
            )

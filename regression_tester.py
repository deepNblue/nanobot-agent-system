...
class RegressionTester:
    ...
    async def run_regression_suite(
        self, test_path: str = "./tests", extra_args: Optional[List[str]] = None, save_results: bool = True
    ) -> Dict:
        """
        运行回归测试套件
        ...
        # 1. 构建命令
        cmd = ["pytest", test_path, "-v", "--tb=short", "--json-report", "--json-report-file=.test_results/report.json"]
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                *cmd, cwd=self.project_path, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            ...
            logger.info(
                f"回归测试完成: " f"passed={results['passed']}, " f"failed={results['failed']}, " f"duration={duration:.2f}s"
            )
            ...
            return {"success": False, "error": str(e), "duration": 0, "results": {}}

    def _parse_test_results(self, output: str) -> Dict:
        """解析测试结果"""
        results = {"total": 0, "passed": 0, "failed": 0, "skipped": 0, "errors": 0, "failures": [], "passes": []}
    ...
    async def capture_baseline(self, test_suite: str = "all") -> Dict:
        ...
        if not result.get("success"):
            return {"success": False, "error": "测试运行失败"}
        ...
        return {"success": True, "baseline": baseline, "file": baseline_file}
    ...
    async def compare_with_baseline(self, test_suite: str = "all") -> Dict:
        """
        与基线对比
        ...
        if not baseline:
            return {"success": False, "error": "未找到基线", "is_baseline": True}
        ...
        if not current_result.get("success"):
            return {"success": False, "error": "当前测试运行失败"}
        ...
        comparison["has_regression"] = (
            comparison["changes"]["failed_diff"] > 0
            or comparison["changes"]["passed_diff"] < 0
            or len(comparison["new_failures"]) > 0
        )
        ...
        comparison["has_improvement"] = (
            comparison["changes"]["failed_diff"] < 0
            or comparison["changes"]["passed_diff"] > 0
            or len(comparison["fixed_tests"]) > 0
        )
    ...
    async def detect_flaky_tests(self, test_path: str = "./tests", runs: int = 5, threshold: float = 0.8) -> List[Dict]:
        """
    ...
    async def run_impact_analysis(self, changed_files: List[str]) -> Dict:
        """
        影响分析 - 确定哪些测试受代码变更影响
                ...
                for changed_file in changed_files:
                    ...
                    if module_name and module_name in content:
                        impacted_tests.append(
                            {"test_file": test_file, "impacted_by": changed_file, "reason": f"导入了 {module_name}"}
                        )
                        break
            except Exception as e:
                logger.error(f"分析文件 {test_file} 失败: {e}")

        return {"changed_files": changed_files, "impacted_tests": impacted_tests, "total_impacted": len(impacted_tests)}
    ...
    def get_trend_analysis(self, days: int = 7) -> Dict:
        ...
        recent_runs = [run for run in self.test_history if datetime.fromisoformat(run["timestamp"]).timestamp() > cutoff]
        ...
        analysis["most_failing"] = sorted(
            [{"test": test, "count": count} for test, count in analysis["failure_frequency"].items()],
    ...
    def generate_report(self, format: str = "markdown") -> str:
        ...
        if format == "json":
            return json.dumps({"history": self.test_history[-10:], "baselines": self.baselines}, indent=2)
...
class TestScheduler:
    ...
    def schedule_test(self, test_path: str, interval_minutes: int = 60, name: str = None):
        """
        调度定期测试

...
class NanobotOrchestratorEnhanced:
    ...
    def __init__(self, base_repo: Optional[str] = None):
        ...
        if self.dashboard_enabled:
            try:
                self.dashboard = start_dashboard(port=self.dashboard_port, tasks_dir=str(self.tasks_dir))
    ...
    async def create_agent_task(
        ...
        worktree_result = self.worktree_manager.create_worktree(
            task_id=task_id, base_branch=base_branch, description=description
        )

        if not worktree_result.get("success"):
            return {"success": False, "error": f"创建Worktree失败: {worktree_result.get('error')}", "step": "worktree"}
        ...
        command = self._render_command_template(
            command_template, task_id=task_id, worktree_path=worktree_path, description=description
        )
        ...
        tmux_result = self.tmux_manager.create_agent_session(
            task_id=task_id, agent_type=agent_type, worktree_path=worktree_path, command=command
        )
        ...
        if not tmux_result.get("success"):
            ...
            return {"success": False, "error": f"创建Tmux会话失败: {tmux_result.get('error')}", "step": "tmux"}
    ...
    async def intervene_task(self, task_id: str, command: str, enter: bool = True) -> Dict:
        """
        干预任务（向运行中的任务发送命令）
        ...
        if not session_name:
            return {"success": False, "error": "任务没有关联的Tmux会话"}

        # 发送命令
        result = self.tmux_manager.send_command(session_name=session_name, command=command, enter=enter)

        # 记录干预操作
        intervention = {"timestamp": datetime.now().isoformat(), "command": command, "result": result}
    ...
    async def capture_task_output(self, task_id: str, lines: int = 100) -> Dict:
        """
        捕获任务输出
        ...
        return self.tmux_manager.capture_pane(session_name=session_name, lines=lines)

    async def complete_task(self, task_id: str, cleanup: bool = False) -> Dict:
        """
        完成任务（可选清理worktree）
        ...
        return {"success": True, "message": f"任务已完成: {task_id}", "cleanup": cleanup}
    ...
    async def list_all_tasks(self) -> List[Dict]:
        ...
        for task_file in self.tasks_dir.glob("*.json"):
            try:
                ...
                tasks.append({"task_id": task_file.stem, "status": "error", "error": str(e)})
    ...
    async def review_code(self, task_id: str) -> Dict:
        ...
        if not pr_number:
            return {"success": False, "error": "未找到对应的Pull Request", "task_id": task_id}
    ...
    async def handle_ci_failure(self, task_id: str) -> Dict:
        ...
        if not ci_status.get("is_failed"):
            return {"success": True, "message": "CI未失败，无需处理", "ci_status": ci_status}

        # 3. 获取运行ID
        run_id = ci_status.get("run_id")
        if not run_id:
            return {"success": False, "error": "无法获取CI运行ID"}
        ...
        if should_retry and self.auto_retry_ci_enabled:
            ...
            if retry_count < 3:  # 最多重试3次
                ...
                if retry_result.get("success"):
                    ...
                    # 监控CI直到完成
                    branch = task_info.get("branch")
                    monitor_result = await self.cicd_integration.monitor_ci_until_complete(branch, timeout=1800, interval=60)
    ...
    async def auto_merge_on_success(self, task_id: str, pr_number: int) -> Dict:
        """
        成功后自动合并PR
        ...
        if not code_review.get("approved"):
            return {"success": False, "error": "代码审查未通过"}
        ...
        if not ci_status.get("is_success"):
            return {"success": False, "error": "CI未通过", "ci_status": ci_status}
        ...
        try:
            ...
            result = subprocess.run(
                cmd, shell=True, cwd=self.worktree_manager.base_repo, capture_output=True, text=True, timeout=60
            )
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "message": f"PR #{pr_number} 已自动合并", "pr_number": pr_number}
            else:
                return {"success": False, "error": f"合并失败: {result.stderr}"}
    ...
    async def _find_pr_for_branch(self, branch: str) -> Optional[int]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, shell=True, cwd=self.worktree_manager.base_repo, capture_output=True, text=True, timeout=30
            )
    ...
    async def full_task_lifecycle(
        self, task_id: str, description: str, agent_type: str = "codex", base_branch: str = "main"
    ) -> Dict:
        """
        ...
        lifecycle_result = {"task_id": task_id, "stages": {}, "success": False}

        try:
            # 1. 创建任务
            print(f"\n[Orchestrator] 阶段1: 创建任务...")
            create_result = await self.create_agent_task(
                task_id=task_id, description=description, agent_type=agent_type, base_branch=base_branch
            )
            ...
            if self.auto_merge_enabled:
                ...
                # 查找PR
                pr_number = await self._find_pr_for_branch(lifecycle_result["stages"]["create"].get("branch"))
    ...
    def _render_command_template(self, template: str, task_id: str, worktree_path: str, description: str) -> str:
        """
        渲染命令模板
        ...
        command = template.format(
            task_id=task_id, worktree_path=worktree_path, description=description.replace("'", "'\"'\"'")
        )
    ...
    def _broadcast_task_update(self, task_id: str, task: Dict):
        ...
        if self.dashboard:
            try:
                self.dashboard.broadcast_task_update(
                    task_id,
                    {
    ...
    def _broadcast_error(self, error_message: str, task_id: Optional[str] = None):
        ...
        if self.dashboard:
            try:
                self.dashboard.broadcast_error({"message": error_message, "task_id": task_id, "error": error_message})
            except Exception as e:
    ...
    async def run_automated_workflow(self, days: int = 7) -> Dict:
        ...
        result = {"started_at": datetime.now().isoformat(), "requirements": [], "tasks": [], "agent_tasks": [], "errors": []}
    ...
    async def auto_create_pr(self, task_id: str) -> Dict:
        ...
        if not self.pr_manager:
            ...
            return {"success": False, "error": "PR管理模块未初始化"}
    ...
    async def complete_task_with_pr(self, task_id: str, cleanup: bool = False) -> Dict:
        """
        ...
        result = {"task_id": task_id, "steps": {}, "success": False}

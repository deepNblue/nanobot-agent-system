...
class CICDIntegration:
    ...
    async def check_ci_status(self, branch: str) -> Dict:
        ...
        try:
            ...
            if result.returncode != 0:
                return {"success": False, "error": f"获取CI状态失败: {result.stderr}", "branch": branch}

            runs = json.loads(result.stdout) if result.stdout else []

            if not runs:
                return {"success": True, "branch": branch, "status": "no_runs", "message": "未找到CI运行记录"}
    ...
    async def get_ci_logs(self, run_id: int, max_lines: int = 500) -> Dict:
        ...
        try:
            ...
            if result.returncode != 0:
                return {"success": False, "error": f"获取运行详情失败: {result.stderr}"}
            ...
            for job in run_details.get("jobs", []):
                if job.get("conclusion") == "failure":
                    ...
                    log_result = subprocess.run(
                        cmd, shell=True, cwd=self.repo_path, capture_output=True, text=True, timeout=60
                    )

                    logs.append({"job_name": job_name, "job_id": job_id, "log": log_result.stdout})
    ...
    async def analyze_ci_failure(self, run_id: int) -> Dict:
        ...
        try:
            ...
            if not logs_result.get("success"):
                return {"success": False, "error": "无法获取CI日志"}
            ...
            for job in logs_result.get("jobs", []):
                ...
                # 提取错误行
                error_patterns = [r"ERROR:.*", r"FAILED.*", r"Error:.*", r"Exception:.*", r"Traceback.*", r"fatal:.*"]
    ...
    async def _analyze_with_glm5(self, error_messages: List[str], logs_result: Dict) -> Dict:
        """
        使用GLM5分析失败原因
...
只返回JSON，不要包含其他内容。"""
    ...
    def _rule_based_analysis(self, error_messages: List[str]) -> Dict:
        ...
        patterns = {
            ...
            "network": [("timeout", "网络超时"), ("connection", "连接失败"), ("503", "服务不可用"), ("502", "网关错误")],
        }
    ...
    async def trigger_ci_retry(self, run_id: int) -> Dict:
        ...
        try:
            ...
            if result.returncode != 0:
                return {"success": False, "error": f"获取运行状态失败: {result.stderr}"}

            run_info = json.loads(result.stdout)

            # 只能重试失败或完成的运行
            if run_info.get("status") not in ["completed"]:
                return {"success": False, "error": f"运行状态不允许重试: {run_info.get('status')}"}
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "run_id": run_id, "message": "CI重试已触发", "timestamp": datetime.now().isoformat()}
            else:
                return {"success": False, "error": f"触发重试失败: {result.stderr}"}

        except Exception as e:
            print(f"[CICD] 触发重试异常: {e}")
            return {"success": False, "error": str(e)}

    async def monitor_ci_until_complete(self, branch: str, timeout: int = 3600, interval: int = 60) -> Dict:
        """
        监控CI直到完成
        ...
        while True:
            ...
            if elapsed > timeout:
                return {"success": False, "error": "监控超时", "elapsed_time": elapsed}
    ...
    async def auto_retry_failed_ci(self, branch: str, max_retries: int = 3) -> Dict:
        """
        自动重试失败的CI
        ...
        while retry_count < max_retries:
            ...
            if not status.get("success"):
                return {"success": False, "error": "无法获取CI状态"}

            # 2. 如果CI成功，直接返回
            if status.get("is_success"):
                return {"success": True, "message": "CI已成功", "retry_count": retry_count}
            ...
            if status.get("is_failed"):
                ...
                if not retry_result.get("success"):
                    return {"success": False, "error": "触发重试失败", "retry_count": retry_count}

                retry_count += 1
                print(f"[CICD] 已触发第 {retry_count} 次重试")

                # 等待CI完成
                monitor_result = await self.monitor_ci_until_complete(branch, timeout=1800, interval=60)  # 30分钟

                if monitor_result.get("is_success"):
                    return {"success": True, "message": f"CI在第{retry_count}次重试后成功", "retry_count": retry_count}
                ...
                # CI正在运行，等待完成
                monitor_result = await self.monitor_ci_until_complete(branch, timeout=1800, interval=60)

                if monitor_result.get("is_success"):
                    return {"success": True, "message": "CI成功", "retry_count": retry_count}

        return {"success": False, "error": f"已达到最大重试次数 {max_retries}", "retry_count": retry_count}

    async def get_workflow_runs(self, workflow_name: Optional[str] = None, limit: int = 20) -> Dict:
        """
        获取工作流运行列表
    ...
    async def notify_ci_status(self, branch: str, status: str, details: Optional[Dict] = None) -> Dict:
        """
        通知CI状态
...
"""
        ...
        return {"success": True, "message": "通知已发送", "timestamp": timestamp}

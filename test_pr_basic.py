...
def test_pr_manager_initialization():
    ...
    # 默认配置
    manager1 = PRManager()
    assert manager1.auto_merge_enabled is False
    assert manager1.merge_method == "squash"
    assert manager1.require_review is True
    print("✅ 默认配置正确")

    # 自定义配置
    config = {"auto_merge": True, "merge_method": "merge", "require_review": False, "min_review_score": 70}
    manager2 = PRManager(config)
    assert manager2.auto_merge_enabled is True
    assert manager2.merge_method == "merge"
    assert manager2.require_review is False
...
def test_get_merge_block_reason():
    ...
    # CI失败
    status2 = {"review": {"approved": True}, "ci": {"success": False}, "mergeable": True, "merge_state": "CLEAN"}
    reason2 = manager.get_merge_block_reason(status2)
    assert "CI" in reason2
    print(f"✅ CI失败: {reason2}")

    # 合并冲突
    status3 = {"review": {"approved": True}, "ci": {"success": True}, "mergeable": False, "merge_state": "DIRTY"}
...
async def test_save_and_load_task():
    ...
    task = {"id": "test_task_12345", "description": "测试任务", "status": "completed"}

...
class CostOptimizer:
    ...
    def __init__(
        self, daily_budget: float = 10.0, hourly_budget: float = 1.0, alert_threshold: float = 0.8, history_file: str = None
    ):
    ...
    async def check_budget(self, model: str, estimated_tokens: int, model_configs: Dict = None) -> bool:
        """检查预算是否充足
        ...
        if self.daily_spend + estimated_cost > self.daily_budget:
            logger.warning(
                f"日预算不足: 当前${self.daily_spend:.4f} + " f"预估${estimated_cost:.4f} > 预算${self.daily_budget:.2f}"
            )
            return False
        ...
        if self.hourly_spend + estimated_cost > self.hourly_budget:
            logger.warning(
                f"时预算不足: 当前${self.hourly_spend:.4f} + " f"预估${estimated_cost:.4f} > 预算${self.hourly_budget:.2f}"
            )
            return False
        ...
        if usage_percentage >= self.alert_threshold:
            logger.warning(f"预算使用率达到 {usage_percentage*100:.1f}% " f"(阈值: {self.alert_threshold*100:.1f}%)")
    ...
    async def record_usage(
        ...
        # 记录
        record = UsageRecord(
            model=model, tokens=tokens, cost=cost, timestamp=datetime.now().isoformat(), task_id=task_id, success=success
        )
        ...
        logger.info(f"记录使用: {model}, tokens={tokens}, cost=${cost:.4f}, " f"今日总计=${self.daily_spend:.4f}")
    ...
    async def optimize_model_selection(self, task: Dict, selector, model_configs: Dict = None) -> Tuple[str, str]:
        """优化模型选择（考虑成本）
        ...
        # 按成本排序
        models_by_cost = sorted(available, key=lambda m: self._get_model_cost(m, model_configs))
    ...
    def get_cost_saving_suggestions(self) -> List[str]:
        ...
        if stats["usage_count"] > 50:
            suggestions.append(f"今日调用次数较多({stats['usage_count']}次)，" "考虑批量处理或使用缓存")
        ...
        if by_model:
            # 找到最贵的模型
            expensive_models = [
                (model, data["total_cost"]) for model, data in by_model.items() if self._get_model_cost(model) > 0.05
            ]

            if expensive_models:
                expensive_models.sort(key=lambda x: x[1], reverse=True)
                model, cost = expensive_models[0]
                suggestions.append(f"模型 {model} 花费较高(${cost:.2f})，" "考虑使用GLM5-Turbo等低成本替代方案")

        # 检查失败率
        total_failures = sum(data["failure_count"] for data in by_model.values())

        if total_failures > 5:
            suggestions.append(f"有{total_failures}次失败调用，" "建议检查提示词质量或增加重试机制")
...
class BudgetAlert:
    ...
    def __init__(self, optimizer: CostOptimizer, webhook_url: str = None, email: str = None):
